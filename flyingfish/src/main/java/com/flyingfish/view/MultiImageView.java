...
import com.flyingfish.util.DensityUtil;
...
public class MultiImageView extends LinearLayout {
	...
	/** 长度 单位为Pixel **/
	private int pxOneMaxWandH=90;  // 单张图最大允许宽高
	...
	private ImageView createImageView(final int position, final boolean isMultiImage) {
		...
		Log.e("TAG", "setList: "+photoInfo);
		Glide.with(getContext())//activty
				.load(photoInfo)
				.asBitmap()
				...
				.into(new SimpleTarget<Bitmap>(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL) {
					...
					public void onResourceReady(Bitmap bitmap, GlideAnimation glideAnimation) {
						...
						if(isMultiImage){
							...
							int expectW = bitmap.getHeight();
							int expectH = bitmap.getWidth();
